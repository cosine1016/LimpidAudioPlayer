...
namespace LAP.Utils
{
    public class BackgroundScanner
    {
        ...
        public LAPP.MediaFile GetTag(string FilePath)
        {
            ...
            {
                return Utility.ToLAPTag(LAPP.MTag.TagReader.GetTag(FilePath), FilePath, null);
            }
            ...
            {
                LAP.Dialogs.LogWindow.Append("Unsupported Tag File : " + FilePath);
                return nulltag;
            }

            LAP.Dialogs.LogWindow.Append("Searching From Cache : " + FilePath);

            for (int i = 0; Config.Library.Files.Count > i; i++)
            {
                if (Config.Library.Files[i].Path == FilePath)
                {
                    if (File.Exists(Config.Library.Files[i].XMLPath))
                    {
                        Album Alb = null;
                        ...
                        for (int ai = 0; Alb.Track.Count > ai; ai++)
                        {
                            ...
                            {
                                if (Alb.Track[ai].LastWriteTime == File.GetLastWriteTime(FilePath).ToString())
                                {
                                    LAP.Dialogs.LogWindow.Append(Alb.Track[ai].ArtworkCachePath);
                                    if (!File.Exists(Alb.Track[ai].ArtworkCachePath))
                                    {
                                    ...
                                    LAP.Dialogs.LogWindow.Append("Found");
                                    return Alb.Track[ai];
                                }
                            }
                        }
            ...
            try
            {
                ...
                if (OTag.Artwork != null)
                {
                    ...
                    if (string.IsNullOrEmpty(Ret.ArtworkCachePath))
                    {
                        ...
                        while (File.Exists(Config.Setting.Paths.Cache + @"\artwork\" + albn + "_" + i + ".jpg"))
                            i++;
                    ...
                    else
                    {
                        ...
                        string path = Config.Setting.Paths.Cache + @"\artwork\" + albn + "_" + i + ".jpg";
                        if (File.Exists(Ret.ArtworkCachePath) == false)
                        {
                            Bitmap bmp = new Bitmap(OTag.Artwork);
                            bmp.Save(path, ImageFormat.Jpeg);
                        }
                ...
                if (File.Exists(AlbumCachePath) == false)
                {
                    Album alb = new Album();
                    alb.Title = Ret.Title;
            ...
            {
                LAP.Dialogs.LogWindow.Append("Tag Reading Error : " + FilePath + "\n" + ex.Message);
                return nulltag;
            }
        }
        ...
        {
            if (File.Exists(Config.Setting.Paths.Cache + @"\artwork\Artwork.xml"))
            {
